...
- **HUD beginner-safe (`BEGINNER_HUD_LOCK`, documento 07):** densidad por defecto del simulador = chart + franja mínima de cuenta/riesgo + acción principal + controles de velocidad, con todos los paneles avanzados colapsados; presupuesto de render = ventana deslizante ~120 velas, 60 fps objetivo, fallback 30 fps documentado en gama baja (la degradación nunca toca exactitud de datos ni ejecución). Sobre el lock, el documento 07 §4.2 define los dos niveles de densidad del MVP: `Beginner HUD` (default, chart-first, sin métricas avanzadas ni datos técnicos visibles) y `Expanded HUD` (estado expandido bajo demanda, no requisito del MVP, no modo profesional), con progressive disclosure transversal, Risk Preview en lenguaje claro antes de confirmar (riesgo estimado, stop loss, tamaño, pérdida máxima aproximada, R/R) y feedback posterior que prioriza decisiones, reglas de riesgo y concepto antes que P/L.
...
- **Feedback estructurado:** proceso primero y P/L después; máximo 3 aciertos + 1 error principal con la vela exacta; el principio detrás; qué practicar después. Siempre cita evidencia del decision log; tono serio, sin humillación, sin hype. Vista inicial resumida, sin tablas extensas; el breakdown completo del score queda colapsado bajo expansión manual (documento 07 §4.2).
